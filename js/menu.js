...
if(viewport.matches){
  ...
  // 한 개 : var nav = document.querySelector('.navigation');
  // 여러개 : var list = document.querySelectorAll('.menu-list');
  var nav = $('.navigation');
  var btn = $('.btn-menubar');
  var menu = $('.menu');
  btn.attr('aria-haspopup', 'true'); /*aria 속성*/ 
  li.addClass('icon-plus');
  item.attr('role', 'button');
  item.attr('aria-haspopup', 'true'); /*aria 속성*/ 
  ...
  btn.click(function(e){
    ...
    if (nav.hasClass('nav-act')) {
      ...
      btn.attr('aria-pressed', 'true'); /* aria 상태정보*/
    } else {
  ...
  item.click(function(e){
    ...
    li.removeClass('icon-minus').addClass();
    item.attr('aria-pressed', 'false');
    item.attr('aria-expanded', 'false');
